...
const update_sliding = (m, stopSpeed) => {
    ...
    if (forward < 0.0 && m.forwardVel >= 0.0) {
        forward *= 0.999 + 0.999 * m.forwardVel / 999.0
    }
    ...
    switch (Mario.mario_get_floor_class(m)) {
        ...
        case SURFACE_CLASS_NOT_SLIPPERY:
            accel = 5.0
            lossFactor = m.intendedMag /100.0 * forward * 0.100 + 0.100
            break
    }
    ...
    // Z speed.
    m.slideVelX += m.slideVelZ * (m.intendedMag / 100.0) * sideward * 100.0
    m.slideVelZ -= m.slideVelX * (m.intendedMag / 200.0) * sideward * 100.0

    const newSpeed = Math.sqrt(m.slideVelX * m.slideVelX + m.slideVelZ * m.slideVelZ)

    if (oldSpeed > 0.0 && newSpeed > 22.0) {
